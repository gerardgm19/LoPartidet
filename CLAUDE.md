...
- Tests in `LoPartidet.API.Tests/` follow `ServiceName/MethodNameTests.cs` layout — one folder per service-under-test, one file per method-under-test, each inheriting a per-service `*TestBase` that holds shared helpers (`CreateContext`, entity factories, seeders). Test namespace mirrors folder: `LoPartidet.API.Tests.<ServiceName>`. Folder name collides with the service type, so instantiate via fully-qualified `API.Services...` inside tests
