...
export default function MatchForm({ matchId }: MatchFormProps) {
  ...
  useEffect(() => {
    if (!editMode || !matchId) return;
    getMatchById(matchId)
      .then((match) => {
        if (!match) {
          showToast(t.matchError);
          setTimeout(() => router.back(), 1200);
          return;
        }
        setSelectedType(match.type);
        setDatetime(new Date(match.date));
        setLocation(match.location);
        setMaxPlayers(match.maxPlayers);
        setDurationInMinutes(match.durationInMinutes);
      })
      .catch(() => {
        showToast(t.matchError);
        setTimeout(() => router.back(), 1200);
  ...
  async function handleSubmit() {
    ...
    try {
      if (editMode && matchId) {
        await updateMatch(matchId, { type: selectedType, date: datetime.toISOString(), location: location.trim(), maxPlayers, durationInMinutes });
      ...
      showToast(editMode ? t.updateMatchError : t.createMatchError);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.navbar}>
        ...
        <Text style={styles.navTitle}>{editMode ? t.editMatchTitle : t.createMatchTitle}</Text>
